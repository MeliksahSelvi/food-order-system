package com.food.order.system.payment.service.outbox.scheduler;
...
public class OrderOutboxHelper {
    ...
    public Optional<List<OrderOutboxMessage>> getOrderOutboxMessageByOutboxStatus(OutboxStatus outboxStatus) {
        return orderOutboxRepository.findByTypeAndOutboxStatus(DomainConstants.ORDER_SAGA_NAME, outboxStatus);
    }

    @Transactional(readOnly = true)
    public Optional<OrderOutboxMessage> getCompletedOrderOutboxMessageBySagaIdAndPaymentStatus(UUID sagaId,
                                                                                               PaymentStatus paymentStatus) {
        return orderOutboxRepository.findByTypeAndSagaIdAndPaymentStatusAndOutboxStatus(DomainConstants.ORDER_SAGA_NAME, sagaId, paymentStatus, OutboxStatus.COMPLETED);
    }
    ...
    public void deleteByOutboxStatus(OutboxStatus outboxStatus) {
        orderOutboxRepository.deleteByTypeAndOutboxStatus(DomainConstants.ORDER_SAGA_NAME, outboxStatus);
    }
